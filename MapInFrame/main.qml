...
//            anchors.fill: parent
            ...
            onMapReadyChanged: {
                ...
                //mapVideo.updateSize()
                //var matTransform = Qt.matrix4x4()
                //updateSize(map, poly, matTransform)
                transf = Qt.createQmlObject(
                            "import QtQuick 2.7;
                             Matrix4x4 {
                                matrix: Qt.matrix4x4();
                            }", video, "");

                //t.matrix = matTransform
                video.transform = transf
                updateSize(map, poly)
            }

            MapMarker {
                id: marker
                coordinate: QtPositioning.coordinate(37.562984, -122.514426)
            }
            ...
            {
                console.log("updateMatrix: " + tl + tr + br + bl + w +h)
            ...
            function updateSize(m, p)
            {
                var tl = m.fromCoordinate(p.path[0], false)
                var tr = m.fromCoordinate(p.path[1], false)
                var br = m.fromCoordinate(p.path[2], false)
                var bl = m.fromCoordinate(p.path[3], false)
...
//                videoContainer.height = sizeh
                ...
                updateMatrix(tlt, trt, brt, blt, video.width, video.height)
